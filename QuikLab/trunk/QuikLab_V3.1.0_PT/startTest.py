...
import casebase.getLatestRev as getLatestRev
...
#     print filedir
    ...
    fp = file(fileUrl ,'wb')
#     print fileUrl
    rev = getLatestRev.getRev('exe').split('\\')[-1]
    revsion = re.findall('(.*?)\.exe',rev)[0]
    print "rev is:"+revsion
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp,title=u'QuikLab3.0自动化测试报告',description="Test revsion:%s"%revsion)
    runner.run(testunit)
    fp.close()
    time.sleep(5)
